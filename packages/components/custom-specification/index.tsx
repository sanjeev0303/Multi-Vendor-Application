...
const CustomSpecification = ({ control, errors }: any) => {
  // Destructure fields and remove function from useFieldArray
  ...
  return (
    <div>
          ...
          <div key={index} className="flex gap-2 items-center">
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  label="Value"
                  placeholder="e.g., 40000mAh, 1.5kg, Plastic"
                  {...field}
                />
              )}
            />
      ...
      {errors?.custom_specifications && (
        <p className='text-red-500 text-xs mt-1'>{errors?.custom_specifications.message as string}</p>
      )}
    </div>
  );
};
